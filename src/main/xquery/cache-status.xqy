...
element div {attribute id {"data"}, text{ xdmp:javascript-eval("xdmp.cacheStatus(xdmp.host())")}},
...
if($debug) then(element h3 {"Group Level Cache Status"}, element pre {element code {xdmp:quote(xdmp:cache-status())}}) else ()
}
